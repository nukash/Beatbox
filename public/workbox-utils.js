importScripts(
  "https://cdn.jsdelivr.net/gh/daffinm/pwa-utils@latest/js/assert.js"
);
...
function WorkboxCacheFileInfo(fileInfoArray) {
  ...
  this.addRevisionParameterTo = function(url) {
    assert.isTrue(
      this.hasRevisionFor(url),
      `Bug: No revision found for ${url}`
    );
...
function WorkboxCacheBeforeCacheOnly(
  serviceWorkerExecutionContext,
  cacheName,
  cacheFileInfoArray
) {
  const CACHE_MATCH_OPTIONS = Object.freeze({
    ignoreSearch: true,
    ignoreVary: true,
  });
    ...
    activationEvent.waitUntil(
      caches
        ...
        .then(function(cache) {
          cache.keys().then(function(keys) {
            for (let response of keys) {
              ...
              let isCachedFileOrphan = !CACHE_FILE_INFO.hasRevisionFor(
                cachedUrl
              );
              if (isCachedFileOrphan) {
                debug.log(
                  `Deleting orphan response from [${CACHE_NAME}]:\n${cachedUrl}`
                );
        ...
        .catch(function(error) {
          debug.error(
            `Problem deleting orphan(s) from cache [${CACHE_NAME}']:\n${error}`
          );
        })
    );
  }
  ...
  async function updateCacheAsNecessary(url) {
    ...
    if (alreadyCached) {
      ...
      let cachedRevision = CACHE_FILE_INFO.getRevisionParameterFrom(
        alreadyCached.url
      );
      ...
      if (!cachedRevision) {
        ...
        assert.isDefined(
          latestRevision,
          "latestRevision",
          `Bug: Cannot find latest revision info for file: ${url}`
        );
